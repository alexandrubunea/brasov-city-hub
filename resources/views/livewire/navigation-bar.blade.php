            ...
            <div id="navbar-collapse" class="hidden lg:block">
                <ul
                    ...
                    @foreach ($tabs as $key => $tab)
                        ...
                        @else
                            <li
                                class="w-full border-b-4 border-b-indigo-700 pb-1 pt-3 pl-3 rounded border-t border-x border-t-transparent border-x-transparent hover:border-x-zinc-200 hover:border-zinc-200 hover:rounded-lg hover:bg-indigo-700 transition-all duration-500 lg:pr-3">
                                <a href="{{ url($tab['route']) }}" class="pr-[100%] lg:pr-0">{{ $key }}</a>
                            </li>
                        @endif
                    @endforeach
